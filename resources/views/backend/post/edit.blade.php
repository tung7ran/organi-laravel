                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="editor_desc" name="desc" rows="4">{{ $post->desc }}</textarea>
                                </div>
                            </div>
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label for="">Nội dung</label>
                                    <textarea name="content" id="editor_desc_product" class="form-control" rows="4">{{$post->content}}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="box box-success">
                ...
                <div class="box-body">
                    @foreach(\App\Models\Post::$postType as $key => $value)
                        <div class="form-group">
                            <label >
                                <input name="type" type="checkbox" class="fomr-control" value="{{ $key }}" {{$post->category_id == $item->id  ? 'selected' : ''}}><span>{{ $value }}</span>
                            </label>
                        </div>
                    @endforeach
                ...
                <div class="box-body checkboxlist">
                    ...
                    <div class="form-group">
                        <label class="custom-checkbox">
                            <input name="category_id" type="checkbox" class="fomr-control" value="{{ $item->id }}" {{$post->category_id == $item->id  ? 'selected' : ''}}><span>{{ $item->name }}</span>
                        </label>
                    </div>
                    @endforeach
                </div>
